...
public:
  ...
  Lexer(std::string finename)
      : lastLocation(
            {std::make_shared<std::string>(std::move(finename)), 0, 0}) {}
  ...
  llvm::StringRef getId() const {
    assert(curTok == tok_identifier &&
           "prereq: getCurToken() == tok_identifier");
    return identifierStr;
  }

  /// Return the current number (prereq: getCurToken() == tok_number)
  double getValue() const {
    assert(curTok == tok_number && "prereq: getCurToken() == tok_number");
    return numVal;
  }

  /// Return the location of the beginning of the current token.
...
private:
  ...
  Token getTok() {
    ...
    if (isdigit(lastChar) || lastChar == '.') {
      ...
      do {
        numStr += lastChar;
        lastChar = Token(getNextChar());
      } while (isdigit(lastChar) || lastChar == '.'); // he-he what about .9 or 9.9.9?
    ...
    if (lastChar == '#') {
      ...
      if (lastChar != EOF) // for eof we will return tok_eof a bit later.
        return getTok();
    }
  ...
  /// If the current Token is a number, this contains its value.
  double numVal = 0;
